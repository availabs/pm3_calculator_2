...
const goldenStandardLottrData = {};

jest.setTimeout(120000);

beforeAll(async done => {
  const csvColMappings = new Map(lottrColMappings);
  ...
  inStream
    ...
    .on('end', () => {
      ...
      sample.reduce((acc, d) => {
        ...
        acc[tmc] = d;
        return acc;
      }, goldenStandardLottrData);

      done();
    });
});

test('Lottr GoldenStandard Tests', done => {
  Promise.all(
    Object.keys(goldenStandardLottrData).map(async tmc => {
      const goldenStandardForTmc = goldenStandardLottrData[tmc];
      ...
      */
      ...
      [...lottrColMappings.values()].forEach(k => {
        expect(
